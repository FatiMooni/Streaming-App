...
class CustomPlaybackPreparer(
    ...
    override fun onPrepareFromMediaId(mediaId: String?, extras: Bundle?) {
        streamingSource.whenReady {
            val itemToPlay: MediaMetadataCompat? = streamingSource.find { item ->
                item.id == mediaId
            }
            if (itemToPlay == null) {
    ...
    override fun onPrepareFromSearch(query: String?, extras: Bundle?) {
        streamingSource.whenReady {
            val metadataList = streamingSource.search(query ?: "", extras ?: Bundle.EMPTY)
    ...
    private fun buildPlaylist(item: MediaMetadataCompat): List<MediaMetadataCompat> =
        streamingSource.filter { it.station == item.station }.sortedBy { it.id }
}
